...
const BASE_PATH = "./circuits/LogicCircuit/";
...
async function generateProof() {
  ...
  const proof = await snarkjs.groth16.prove(
    BASE_PATH + "out/LogicCircuit.zkey",
    BASE_PATH + "out/circuit.wtns"
  )
...
async function main() {
  // deploy contract
  const Verifier = await ethers.getContractFactory("./contracts/LogicCircuitVerifier.sol:Verifier");
